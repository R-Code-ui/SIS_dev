...
export default function Index({ messages, filters }) {
    ...
    const applyFilters = () => {
        router.get(route('student.messages.index'), { search, type, sort }, {
            preserveState: true,
            preserveScroll: true,
        });
    };
        ...
        },
        {
            ...
            render: (item) => (
                <div className="flex space-x-2">
                    <Button variant="primary" onClick={() => router.visit(route('student.messages.show', item.id))}>View</Button>
    ...
    const confirmDelete = () => {
        ...
        router.delete(route('student.messages.destroy', selectedMessage.id), {
            onSuccess: () => { setDeleteModal(false); setSelectedMessage(null); }
        });
    };

    return (
        <StudentLayout header="Messages">
            <Head title="Messages" />
            <div className="py-6">
                <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                    ...
                    <div className="mb-4 flex flex-wrap gap-4 items-end bg-white p-4 rounded-lg shadow">
                        <div className="flex-1 min-w-[200px]">
                            ...
                            <input type="text" placeholder="Subject, sender, receiver..." value={search}
                                onChange={e => setSearch(e.target.value)} onKeyDown={handleSearchKeyDown}
                                className="w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500" />
                        </div>
                        <div className="w-48">
                            <label className="block text-sm font-medium text-gray-700 mb-1">Type</label>
                            <select value={type} onChange={e => { setType(e.target.value); setTimeout(applyFilters, 0); }}
                                className="w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500">
