...
export default function Index({ lessons, classes, subjects, filters = {} }) {
    ...
    const applyFilters = () => {
        router.get(route('admin.lessons.index'), {
            search, class_id: classId, subject_id: subjectId, sort,
    ...
    const handleEdit = (lesson) => router.visit(route('admin.lessons.edit', lesson.id));
    const handleDelete = (lesson) => { setSelectedLesson(lesson); setDeleteModal(true); };
    const confirmDelete = () => {
        if (!selectedLesson) return;
        router.delete(route('admin.lessons.destroy', selectedLesson.id), {
            onSuccess: () => { setDeleteModal(false); setSelectedLesson(null); }
        });
    };

    const lessonsData = lessons?.data ?? [];
    const links = lessons?.links ?? [];
            ...
            <div className="py-6">
                <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                    <div className="mb-4 flex justify-between items-center">
                        ...
                        <Link href={route('admin.lessons.create')}><Button variant="primary">Add New Lesson</Button></Link>
                    </div>
                    ...
                    <DataTable columns={columns} data={lessonsData} actions={true} onEdit={handleEdit} onDelete={handleDelete} />
