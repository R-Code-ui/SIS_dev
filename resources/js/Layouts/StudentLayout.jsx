...
export default function StudentLayout({ header, children }) {
    ...
    const navigation = [
        ...
        { name: 'Events', href: route('student.events.index'), current: route().current('student.events.*') },
        { name: 'Messages', href: route('student.messages.index'), current: route().current('student.messages.*') },
        { name: 'Announcements', href: route('student.announcements.index'), current: route().current('student.announcements.*') },
        { name: 'My Classes', href: route('student.classes.index'), current: route().current('student.classes.*') },
    ];

    return (
        <div className="flex h-screen overflow-hidden bg-gray-100">
            {/* Sidebar – same green styling as admin/teacher */}
